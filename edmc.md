...
| `VERSION` and `__version__` on `load.py` | yes, 4.0.1 | yes, `v0.4.400.3.005` |
| Semantic version | 4.0.1 | **no** - four parts and a leading `v` |
| Ships only what it needs | `lib/` is gitignored | `backup_legacy/` untracked |
| Published release to point a zip at | v4.0.1, zip attached | yes, v0.4.400.3.005 |
...
`plugins/RhinoSpotter.json` for 4.0.1, the same fields as
